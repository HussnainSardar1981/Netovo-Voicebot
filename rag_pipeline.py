...
class RAGPipeline:
    ...
    def _build_args_for_stage(self, stage, mode, target=None):
        """
        ...
        if mode == 'all':
            ...
            if stage == 'download':
                ...
                args = []  # extraction.py processes all by default
            ...
            if stage == 'download':
                ...
                args = ['--customers', customer_id]  # Note: plural form
            elif stage in ('clean', 'chunk', 'embed', 'index'):
                args = ['--customer', customer_id]
            ...
            if stage == 'download':
                ...
                # Extract doesn't support single file - would need PDF path
                logger.warning("Extract stage not supported for single file mode (needs PDF)")
                return None
            elif stage == 'clean':
                args = ['--file', str(file_path)]
            elif stage in ('chunk', 'embed', 'index'):
                args = ['--file', str(file_path)]

        return args

    def run_stage(self, stage, mode, target=None):
        """
        Run a single stage
        ...
        args = self._build_args_for_stage(stage, mode, target)
        if args is None:
    ...
    def run_all_customers(self, skip_stages=None, only_stages=None):
        ...
        for stage in stages:
            success = self.run_stage(stage, mode='all')
    ...
    def run_single_customer(self, customer_id, skip_stages=None, only_stages=None):
        ...
        for stage in stages:
            success = self.run_stage(stage, mode='customer', target=customer_id)
            if not success and self.results.get(stage) != 'skipped':
...
Note: Single file mode automatically skips download/extract stages
    ...
    parser.add_argument(
        '--file',
        type=str,
        help='Run for a specific file (skips download/extract)'
    )
    parser.add_argument(
        '--customer-id',
        type=str,
        help='Customer ID (for use with --file if not in path)'
    )
    parser.add_argument(
        '--skip',
        nargs='+',
