...
class CleaningPipeline:
    ...
    def __init__(self, server_root: Path):
        ...
        if not any(isinstance(h, logging.FileHandler) for h in logger.handlers):
            file_handler = logging.FileHandler(str(logs_dir / 'cleaning.log'), encoding='utf-8')
            file_handler.setFormatter(logging.Formatter('%(asctime)s - CLEANER - %(levelname)s - %(message)s'))
...
Examples:
  ...
  python clean_documents_server.py --customers CUST1 CUST2
    ...
    parser.add_argument('--server-root', default=str(PathsConfig.RAG_DIR),
                       help='RAG root directory (default: from paths_config.py)')
