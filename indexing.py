...
def index_all_customers(indexer) -> Dict:
    """
    ...
    customers_dir = PathsConfig.CUSTOMERS_DIR
...
Examples:
  ...
  python indexing.py --query "What is 3CX?" --collection stuart_dean  # Query collection
    ...
    try:
        ...
        if args.customer:
            customers_dir = PathsConfig.get_customer_dir(args.customer)
            if not customers_dir.exists():
        ...
        results = index_all_customers(indexer)
